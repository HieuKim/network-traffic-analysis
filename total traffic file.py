...
for index_file in range(1,index_max+1,1):
    ...
    file_name = "/home/mpec/hust_traffic_pcap/111-120/test_111-120_splited" + str(index_file)+ '.pcap'
    print("Working with file: ", index_file, file_name)
    matrix_ip_sport_len = np.empty([1, 3])
    ...
    if index_file == 1:
        ...
        with open('test111-120.csv', 'a') as f:
            np.savetxt(f, matrix_ip_sport_len, delimiter=',', fmt='%d')  # ip_src,s_port,length,ip_dst,d_port,proto
end = time.time()
